    ...
    def prepare(self):
        ...
        for i, img_id in enumerate(ids):
            ...
            for id in list(persons_ids):
                ...
                # print("[DEBUG] keypoints shape: ", str(len(person_meta["keypoints"])), ", check one keypoints:",
                #       str(person_meta["keypoints"]))
            ...
            if len(persons) > 0:
                ...
                # return a list of all joints' coordinate [x,y]
                main_person.all_joints = JointsLoader.from_coco_keypoints(keypoints, w, h)
                # a list contains all main person's params
                self.all_meta.append(main_person)
                # print("[DEBUG] joints shape: %d, joints list: %s" % (
                # len(main_person.all_joints[0]), str(main_person.all_joints)))
