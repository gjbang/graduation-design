...
class FlipTransform(Transform):
    ...
    def apply_image(self, meta):
        ...
        #         print("[DEBUG] new image dim: ", str(new_img.ndim))
        #         new_img = new_img[:, :, np.newaxis]
        #     new_mask = None if mask is None else cv2.flip(mask,1)
        #     if mask is not None:
        ...
        #
        # return result
        # cv2.flip: 0: vertical flip; 1: horizon flip; -1: both
            new_img = cv2.flip(img, 1)
            # adjust dim2 -> dim3; 单通道时会出现降维，需要变为三维数据；多通道时不会出现降维
            if img.ndim == 3 and new_img.ndim == 2:
                new_img = new_img[:, :, np.newaxis]
            new_mask = None if mask is None else cv2.flip(mask, 1)
