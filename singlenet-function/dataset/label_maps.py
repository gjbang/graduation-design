...
# values of params in order:
# [19, 28, 28, list, 5, 8]
# height * stride == 224 -> image input size
# joints' range is over than 224, so they are the original coordinates of coco images
def create_heatmap(num_maps, height, width, all_joints, sigma, stride):
    ...
    # print("[DEBUG] create heatmap: sigma: %s, stride: %s, height: %s, width: %s" %(sigma,stride,height,width))

    max_value=0
    for joints in all_joints:
        # plane_idx is the index of joint in joints list
        # joint is (x,y) coordiniates of one joint indexing by plane_idx
        for plane_idx, joint in enumerate(joints):
            # print(joint)
            if joint:
                # if max(joint[0],joint[1]) > max_value:
                #     max_value = max(joint[0],joint[1])
                # print(joint)
    ...
    # print("[DEBUG] check max value of coordinate of joints: %d " % max_value)
