...
/** \internal \returns a < b or a==NaN or b==NaN as a bit mask */
        ...
        template<>
        EIGEN_DEVICE_FUNC inline float pzero<float>(const float &a) {
            EIGEN_UNUSED_VARIABLE(a)
            return 0.f;
        }

        template<>
        EIGEN_DEVICE_FUNC inline double pzero<double>(const double &a) {
            EIGEN_UNUSED_VARIABLE(a)
            return 0.;
        }

        template<typename RealScalar>
        EIGEN_DEVICE_FUNC inline std::complex<RealScalar>

        ptrue(const std::complex <RealScalar> & /*a*/) {
            RealScalar b;
            b = ptrue(b);
        ...
        bitwise_helper(const Packet &a, const Packet &b, Op op) {
...
/** \internal \returns \a or \b for each field in packet according to \mask */
        ...
        template<>
        EIGEN_DEVICE_FUNC inline float pselect<float>(
                const float &cond, const float &a, const float &b) {
            return numext::equal_strict(cond, 0.f) ? b : a;
        }

        template<>
        EIGEN_DEVICE_FUNC inline double pselect<double>(
 ...
 * rounding mode */
        ...
        print(const Packet &a) { using numext::rint; return rint(a); }

/** \internal \returns the ceil of \a a (coeff-wise) */
        template<typename Packet> EIGEN_DECLARE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS
                Packet

        pceil(const Packet &a) { using numext::ceil; return ceil(a); }
