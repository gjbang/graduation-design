...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE PacketT load(Index i, Index j) const {
    ...
    struct storePacketBlock_helper {
        ...
        void store(const blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType, Incr> *sup, Index i, Index j,
    ...
    struct storePacketBlock_helper<SubPacket, std::complex < float>, n, idx>
    {
        ...
        store(const blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType, Incr> *sup, Index i, Index j,
              const PacketBlock <SubPacket, n> &block) const {
            spbh.store(sup, i, j, block);
            for (int l = 0; l < unpacket_traits<SubPacket>::size; l++) {
    ...
    struct storePacketBlock_helper<SubPacket, ScalarT, n, -1> {
        ...
        void store(const blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType, Incr> *, Index, Index,
    ...
    struct storePacketBlock_helper<SubPacket, std::complex < float>, n, -1>
    {
        EIGEN_DEVICE_FUNC
        EIGEN_ALWAYS_INLINE void
        store(const blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType, Incr> *, Index, Index,
              const PacketBlock <SubPacket, n> &) const {
        }
    };
